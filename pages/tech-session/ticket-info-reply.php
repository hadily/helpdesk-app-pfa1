          ...
          <div class="card mb-4">
            ...
            <div class="card-body px-0 pt-0 pb-2">
              ...
              <?php
                ...
                $url = "../includes/ticket-reply.php?data=".urlencode($data);

                echo"<form role='form' action='$url' method='POST'>";

                // echo $_SESSION['id_ticket']; //works
                // echo $_SESSION['name']; //works 

                // get data from ticket table 
                $query = "SELECT * FROM ticket WHERE num = $data";
                $result = mysqli_query($conn, $query);
                $row = mysqli_fetch_assoc($result);

                //create a global variable for state
                $_SESSION['state'] = $row['state'];
                //echo $_SESSION['state']; //works

                // NB : no need to submit id_tech with the form, I only need the reply field :)
                ...
                if ($row['state'] == 0) {
                  ...
                  <textarea class='form-control form-control-lg ticket-msg' name='reply' aria-label='reply' rows='8' cols='50'>
                  </textarea>
                </div>
                <div class='text-center'>
                  <button type='submit' class='btn btn-lg btn-primary btn-lg ticket-btn'>Submit</button>
                </div>";
